...
\begin{abstract}
\vspace{-.5em}
Domain generalization (DG) asks a model to learn from several labeled source domains and generalize to a previously unseen target domain without target labels. Modern DG evaluation has made this problem more exacting: simple empirical risk minimization and post-hoc averaging baselines are already strong, so a new method must explain not only that averaging helps, but which checkpoints should be averaged and why their average should remain stable under domain shift. We study this source-only post-hoc setting. A training run has already produced a checkpoint bank, and the remaining deployment problem is to retain a subset and choose averaging weights without seeing target data. We propose \emph{distributional canalization}, a local source-supported view of target shift that yields a certificate with three terms: source fit, sensitivity to source-mixture perturbations, and a mergeability remainder for the deployed weight-space soup. This certificate motivates \cda{}, a deployment pipeline that compresses the checkpoint bank by preserving source-support disagreement structure, then weights the retained family by guarded pivotality or by a balanced target-set projection. Across five standard DG benchmarks, \cda{} obtains \(89.1\) on \data{PACS}, \(79.5\) on \data{VLCS}, \(72.9\) on \data{OfficeHome}, \(52.3\) on \data{TerraIncognita}, and \(47.8\) on \data{DomainNet}, for a \(68.3\) average compared with \(66.9\) for \swad{} and \(68.0\) for \diwa{} in the same comparison package. On the \data{PACS} SWAD-style comparison, \cda{} reaches \(89.1\) out-of-domain and \(97.9\) in-domain accuracy. \prov{A broader diagnostic layer indicates that CDA averages occupy flatter local source-loss regions than ERM and uniform late-window soups, reduce source-mixture flatness by \(0.44\) relative units, cut checkpoint-selection fluctuation by \(76.8\%\), and preserve the gains when applied on top of CORAL-style and SAM-style checkpoint banks.} These results support a unified view of post-hoc DG: the deployable object is not merely a late-training average, but a certificate-guided checkpoint family whose centered source-domain responses, flatness, and mergeability jointly determine robustness to hidden source-mixture shift.
\end{abstract}
...
Table~\ref{tab:intro_summary} summarizes the empirical position. CDA reaches a \(68.3\) average across \data{PACS}, \data{VLCS}, \data{OfficeHome}, \data{TerraIncognita}, and \data{DomainNet}, compared with \(66.9\) for SWAD and \(68.0\) for DiWA in the comparison package. \prov{The expanded analysis in Sections~\ref{sec:method} and~\ref{sec:experiments} further shows that this gain is accompanied by lower local source flatness, lower source-mixture flatness, lower checkpoint-selection fluctuation, and stronger performance when CDA is applied to alternate checkpoint banks.}

\paragraph{Relationship to prior DG work.}
DG methods have evolved through invariant representation learning, adversarial alignment, meta-learning, distributional robustness, style augmentation, and causal or invariance-oriented objectives \citep{muandet2013domain,ganin2016domain,li2018learning,arjovsky2019invariant,Sagawa2020GroupDRO,zhou2021domain}. CDA does not compete with those methods by changing the training objective. It competes at deployment time: given a checkpoint bank, it chooses a family and a weight vector. This makes it closest to SWA, SWAD, model soups, DiWA, and EoA \citep{izmailov2018swa,cha2021swad,wortsman2022soups,rame2022diwa,arpit2022eoa}. The distinguishing feature is the source-mixture certificate. Flatness matters because it reduces local deployment sensitivity; diversity matters when centered source-domain responses cancel; averaging matters only when the soup remains mergeable.
...
The theory depends on a source-supported hidden-shift assumption. If the target shift is driven by mechanisms that are not well approximated by nearby redistributions of source-domain mass, then the CDA certificate can become loose or irrelevant. The ambiguity set is local, and its usefulness is tied to that locality. This is the same kind of limitation that appears in any source-only DG guarantee: without assumptions linking the target to the sources, target-domain performance cannot be inferred from source-domain data alone.
