...
\title{STAWA-New: Domain Generalization by Seeking Functional Stationarity Within a Low-Loss Plateau}
...
\begin{abstract}
This manuscript records the plateau-constrained \textbf{STAWA-New} revision.
Single-run post-hoc methods matter in domain generalization because they can be attached to strong empirical risk minimization pipelines without retraining. The canonical method in this regime is \swad, which selects and averages dense checkpoints by seeking flat minima in parameter space. We argue that flatness is only an indirect proxy for the label-free domain generalization problem. Among comparably well-fit checkpoints, a useful DG selector should prefer those that are \emph{functionally stationary} under the actual ongoing stochastic training dynamics that produced them. This principle leads to Stationarity-Aware Weight Averaging (\stawa), a single-run retrospective post-hoc method that first restricts attention to a low-loss source-validation plateau and then ranks checkpoints inside that plateau by the mean and spread of short-horizon predictive drift on a held-out support set. We show that functional drift controls the distance from a checkpoint to its local prediction plateau, that the drift mean and variance together upper-bound hidden unstable subsets and density-ratio shifts, and that the empirical \stawa{} components concentrate uniformly over dense checkpoint sequences. We then derive a target-risk certificate in which source-fit slack, mean drift, and drift variance are the controllable terms, prove finite-sample recovery of plateau membership and within-plateau stationarity ranking, and prove a constructive separation theorem: there exist checkpoints with identical validation loss and identical local flatness statistics that any flatness-only selector treats as equivalent, while \stawa{} strictly prefers the checkpoint with lower target risk. Finally, we justify dense checkpointing and low-drift window averaging under local regularity assumptions. The manuscript includes rigorous proofs, analytical figures, an executable protocol, and published baseline results from prior work; all new \stawa{} quantitative results remain intentionally marked as \TBD{} pending execution.
\end{abstract}
...
Our answer is \stawa{}, Stationarity-Aware Weight Averaging. The principle is:
\begin{quote}
    Among comparably well-fit checkpoints, those whose predictions have largely stopped drifting under the ongoing stochastic training dynamics are better candidates for robust post-hoc averaging under shift.
\end{quote}
Instead of probing the loss landscape in parameter space, \stawa{} probes the trajectory in \emph{function space}. At checkpoint $\theta_t$, it measures how much the predictions of $\theta_t$ change over the next $H$ checkpoints on a held-out support set, first restricts attention to checkpoints whose source validation loss already lies inside a near-optimal plateau, and then averages a contiguous low-drift window inside that plateau. The method is intentionally retrospective: it is designed to evaluate checkpoints inside one completed trajectory, not to serve as an online stopping rule.
...
\begin{enumerate}[leftmargin=1.4em]
    \item We introduce \stawa{}, a label-free, single-run post-hoc DG method that first gates checkpoints by near-best validation loss and then ranks the remaining plateau by mean and spread of short-horizon predictive drift.
    ...
    \item We prove uniform concentration of the empirical \stawa{} components over dense checkpoint sequences, derive an explicit target-risk certificate whose controllable terms are source-fit slack, drift mean, and drift spread, and show finite-sample recovery of plateau membership and within-plateau stationarity ranking.
...
\begin{figure}[t]
    ...
    \caption{\textbf{Conceptual shift from flatness to functional stationarity.} \swad{} probes geometric robustness in weight space. \stawa{} asks whether the predictor itself has entered a stable functional plateau under the actual training dynamics. Two checkpoints can have similar validation loss and similar flatness while still differing strongly in future predictive drift.}
    \label{fig:concept}
\end{figure}
...
\end{equation}
be the labeled source-validation loss at checkpoint $t$. The plateau-internal stationarity score is
\begin{equation}
    J_t
    \triangleq
    \lambda_1 \mu_t
...
\begin{equation}
    L_V^\star \triangleq \min_{1\le t\le K-H} L_V(\theta_t)
\end{equation}
and the loss-admissible plateau
\begin{equation}
    \mathcal{A}_{\epsilon}
    \triangleq
    \left\{
        t : L_V(\theta_t) \le L_V^\star + \epsilon
    \right\},
    \label{eq:plateau_set}
\end{equation}
where $\epsilon \ge 0$ is a source-fit tolerance. This gate is the practical fix to the main failure mode of unconstrained stationarity selection: an early checkpoint can be very calm and still be materially underfit. \stawa{} therefore enforces source fit first and uses stationarity only inside a near-optimal validation plateau.

\paragraph{Analysis score.}
For theory it is convenient to retain the composite quantity
\begin{equation}
    S_t
    \triangleq
    L_V(\theta_t) + J_t,
...
\end{equation}
but the actual selector minimizes $J_t$ only after the loss gate in Eq.~\eqref{eq:plateau_set} has been imposed.
...
Let
\begin{equation}
    t^\star \in \argmin_{t\in \mathcal{A}_{\epsilon}} J_t.
\end{equation}
Define the plateau-constrained low-drift set
\begin{equation}
    \widetilde W_{\epsilon,\delta}
    \triangleq
    \left\{
        t \in \mathcal{A}_{\epsilon} : J_t \le J_{t^\star} + \delta
    \right\},
\end{equation}
restricted to the connected component containing $t^\star$. If that component has fewer than $M$ checkpoints, we enlarge it symmetrically by nearest neighbors inside $\mathcal{A}_{\epsilon}$ until it contains $\min(M,|\mathcal{A}_{\epsilon}|)$ checkpoints. The resulting averaging set is denoted $W_{\epsilon,\delta,M}$. The final model is the weight average
\begin{equation}
    \bar \theta_{\stawa}
    \triangleq
    \sum_{t\in W_{\epsilon,\delta,M}} w_t \theta_t,
    \qquad
    w_t \ge 0,
    \quad
    \sum_{t\in W_{\epsilon,\delta,M}} w_t = 1.
\end{equation}
...
\begin{figure*}[t]
    ...
    \caption{\textbf{\stawa{} pipeline.} Starting from a single dense training trajectory, \stawa{} computes short-horizon predictive drift on a held-out support set, filters checkpoints to a near-best validation plateau, ranks the remaining checkpoints by drift mean and spread, and averages a contiguous plateau-constrained low-drift window. No domain labels are needed at any stage.}
    \label{fig:pipeline}
\end{figure*}
...
\begin{algorithm}[t]
    ...
    \label{alg:stawa}
    \begin{algorithmic}[1]
        \Require Checkpoints $\{\theta_t\}_{t=1}^{K}$ from one training run; held-out support set $U$; labeled validation set $V$; horizon $H$; weights $\lambda_1,\lambda_2$; loss tolerance $\epsilon$; drift width $\delta$; minimum window size $M$
        \For{$t=1,\dots,K-H$}
            \For{each $x_i \in U$}
        ...
        \EndFor
        \State $L_V^\star \gets \min_t L_V(\theta_t)$
        \State $\mathcal{A}_{\epsilon} \gets \{ t : L_V(\theta_t) \le L_V^\star + \epsilon\}$
        \State $t^\star \gets \argmin_{t\in \mathcal{A}_{\epsilon}} J_t$
        \State $W_{\epsilon,\delta,M} \gets$ connected plateau-constrained low-drift window around $t^\star$, enlarged to width $M$ if needed
        \State \Return $\bar \theta_{\stawa} = \sum_{t\in W_{\epsilon,\delta,M}} w_t \theta_t$
    \end{algorithmic}
\end{algorithm}
...
\begin{enumerate}[leftmargin=1.4em]
    ...
    \item the empirical components concentrate and recover plateau membership plus within-plateau ranking,
    \item flatness-only selectors can be blind to the signal \stawa{} measures.
\end{enumerate}

The theory target is deliberately modest. We do not try to prove that low drift universally implies good domain generalization. Instead, we analyze \stawa{} as a \emph{selector within one trajectory}: among checkpoints with comparable source fit, does smaller short-horizon functional instability produce a tighter target-risk certificate and a more informative ranking signal than flatness alone?
...
Proposition~\ref{prop:concentration} shows that the two ingredients used by \stawa{} are estimable uniformly over dense checkpoint sequences: validation loss for the plateau gate and stationarity for ranking inside that gate.

\begin{proposition}[Finite-sample recovery of plateau membership]
...
\begin{equation}
    L_V^\star \triangleq \min_t R_V(p_t),
\end{equation}
the population plateau
\begin{equation}
    \mathcal{A}_{\epsilon}^\star
    \triangleq
    \{ t : R_V(p_t) \le L_V^\star + \epsilon\},
\end{equation}
and the empirical plateau
\begin{equation}
    \widehat{\mathcal{A}}_{\epsilon}
    \triangleq
    \{ t : \hat L_V(\theta_t) \le \hat L_V^\star + \epsilon\},
    \qquad
    \hat L_V^\star \triangleq \min_t \hat L_V(\theta_t).
\end{equation}
Then with probability at least $1-\delta$, simultaneously for all $t$:
\begin{align}
    R_V(p_t) \le L_V^\star + \epsilon - 2\eta_{n,\delta}
    &\Longrightarrow
    t \in \widehat{\mathcal{A}}_{\epsilon}, \\
    R_V(p_t) \ge L_V^\star + \epsilon + 2\eta_{n,\delta}
    &\Longrightarrow
    t \notin \widehat{\mathcal{A}}_{\epsilon}.
\end{align}
\end{proposition}

\begin{corollary}[Finite-sample recovery of within-plateau stationarity ranking]
\label{cor:ranking}
Under the assumptions of Proposition~\ref{prop:concentration}, define
\begin{equation}
    \eta_{J,m,\delta}
...
\begin{equation}
    s,t \in \mathcal{A}_{\epsilon-2\eta_{n,\delta}}^\star
    \qquad\text{and}\qquad
    J_s^\star + 2\eta_{J,m,\delta} < J_t^\star,
\end{equation}
then with probability at least $1-\delta$,
\begin{equation}
    s,t \in \widehat{\mathcal{A}}_{\epsilon}
    \qquad\text{and}\qquad
    \hat J_s < \hat J_t.
\end{equation}
\end{corollary}

Corollary~\ref{cor:ranking} is the selector-consistency statement missing from purely philosophical stability arguments: once source-fit membership is recovered, the empirical post-hoc procedure ranks checkpoints correctly by stationarity inside the plateau whenever the population stationarity gap is large enough.
...
Equation~\eqref{eq:certificate} is the main theoretical justification of \stawa{}. The uncontrollable term is still the source-target shift $\Gamma_t$. But among the terms the selector can influence, source-fit slack, mean drift, and drift spread are exactly the right objects. In particular, if $t\in\mathcal{A}_{\epsilon}^\star$, then $\hat R_V(p_t)$ is already constrained to lie near the best source-validation risk up to the plateau tolerance $\epsilon$ and estimation error, so the selector's remaining job is to minimize stationarity inside that plateau.
...
\begin{itemize}[leftmargin=1.4em]
    ...
    \item \textbf{\stawa-Window}: contiguous plateau-constrained low-drift averaging (default).
\end{itemize}
...
\begin{figure}[t]
    ...
    \caption{\textbf{Dense checkpointing and low-drift window selection.} \stawa{} preserves the same practical scaffold that made \swad{} attractive: dense checkpointing and averaging within one connected plateau-constrained low-drift region. The difference is that the selector targets functional stationarity rather than parameter-space flatness.}
    \label{fig:window}
\end{figure}
...
\begin{table}[t]
    ...
    \begin{tabular}{lc}
        ...
        Single checkpoint vs. plateau-constrained low-drift window & \TBD \\
...
The theory proves six concrete points. First, mean predictive drift controls the risk gap between a checkpoint and its local prediction plateau. Second, the variance term controls hidden unstable subsets and bounded density-ratio shifts. Third, the empirical validation and stationarity components concentrate uniformly over dense checkpoint sequences. Fourth, finite samples recover plateau membership and within-plateau stationarity ranking whenever the population margins are large enough. Fifth, the target-risk certificate depends on source-fit slack, mean drift, and drift spread as the controllable terms. Sixth, flatness-only selectors can be provably blind on an explicit class of examples.
...
We proposed \stawa{}, a label-free single-run retrospective DG selector based on functional stationarity rather than pooled flatness. The method first restricts to a near-best source-loss plateau, then ranks that plateau by mean and spread of short-horizon predictive drift, and finally averages a contiguous plateau-constrained low-drift window. Our analysis shows that low drift places a checkpoint near its local plateau, that variance controls hidden unstable subsets, that the empirical components concentrate and recover plateau membership plus within-plateau ranking, and that flatness-only selectors can miss target-relevant instability that \stawa{} captures. The empirical results remain to be run, but the manuscript now contains the method, the theory, the figures, the protocol, and the published baselines needed to test whether functional stationarity is a genuinely new selector principle rather than a repackaging of late-phase checkpoint smoothing.
...
\end{equation}
Combining the bounds on $\hat\mu_t$ and $\hat\sigma_t$ proves Eq.~\eqref{eq:conc_j}; Eq.~\eqref{eq:conc_loss} is already the validation-loss component bound needed for the plateau gate.
\end{proof}
